 ...
 *
 */
class Tsr1000 : public PingSensor {
    Q_OBJECT
public:
    Tsr1000();
    ~Tsr1000();
    ...
    {
        tsr1000_set_gain_setting m;
        m.set_gain_setting(gain_setting);
        m.updateChecksum();
        writeMessage(m);
        request(Tsr1000Id::GAIN_SETTING);
    }
     ...
     */
    enum ContinuousId : uint16_t { PROFILE = Tsr1000Id::PROFILE };
    ...
    {
        tsr1000_set_ping_interval m;
        m.set_ping_interval(ping_interval);
        m.updateChecksum();
        writeMessage(m);
        request(Tsr1000Id::PING_INTERVAL);
    }
     ...
     */
    Q_INVOKABLE void emitPing() const { request(Tsr1000Id::PROFILE); }
...
private:
    ...
    QMap<QString, settingsConfiguration> _pingConfiguration {
        {{"1_pingInterval"},
            {66, 20, 1000, std::bind(&Tsr1000::ping_interval, this),
                [this](long long int value) { set_ping_interval(value); }}},
        {{"1_speedOfSound"},
            {1500000, 50000, 10000000, std::bind(&Tsr1000::speed_of_sound, this),
                [this](long long int value) { set_speed_of_sound(value); }}},
        {{"2_automaticMode"},
            {true, false, true, std::bind(&Tsr1000::mode_auto, this),
                [this](long long int value) { set_mode_auto(value); }}},
        {{"3_gainIndex"},
            {0, 0, 6, std::bind(&Tsr1000::gain_setting, this), [this](long long int value) { set_gain_setting(value); }}},
        {{"3_lengthDistance"},
            {5000, 500, 120000, std::bind(&Tsr1000::length_mm, this),
                [this](long long int value) { set_length_mm(value); }}},
        {{"3_startDistance"},
            {0, 0, 120000, std::bind(&Tsr1000::start_mm, this), [this](long long int value) { set_start_mm(value); }}},
    };
    ...
    friend QDebug operator<<(QDebug d, const Tsr1000::messageStatus& other);
